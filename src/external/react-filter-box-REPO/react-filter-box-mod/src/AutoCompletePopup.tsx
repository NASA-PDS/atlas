...
export default class AutoCompletePopup {
    ...
    constructor(
        ...
        private needAutoCompletevalues: (text: string) => Promise<HintInfo[]>
    ) {
        this.doc = cm.getDoc()
    ...
    private processText(value: string | Object, self: HintResult, data: Completion): any | Object {
        if (!_.isString(value)) {
            return value
        }
        if (data.type === 'category') return value + ':'
        if (data.type === 'value') {
            if (value.includes(' ')) {
                value = `"${value}"`
            }
    ...
    private onPick(cm: ExtendedCodeMirror, self: HintResult, data: Completion) {
        ...
        cm.replaceRange(this.processText(value, self, data), self.from, self.to, 'complete')
    }
    ...
    private buildCompletionObj(info: HintInfo): Completion {
        ...
        if (info.count != null) completion.count = info.count
        return completion
    }
    ...
    async show() {
        return new Promise(async (resolve, reject) => {
            ...
            this.hintOptions.hintValues = await this.needAutoCompletevalues(text)
